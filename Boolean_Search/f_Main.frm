...
Begin VB.Form f_Main 
   ...
   Begin VB.Label Top_Caption 
      BackStyle       =   0  'Transparent
      Caption         =   "Boolean Search [ Example ] by XIII"
      ForeColor       =   &H00808080&
      Height          =   255
      Left            =   120
      TabIndex        =   11
...
Private Function Build_Boolean_Criteria(ByVal Temp_STR As String) As String

Dim Filter_String As String
Dim Temp_STR_2 As String
Dim Search_Words
Dim Like_Type As String
Like_Type = "%"

Temp_STR_2 = Replace(Temp_STR, "  ", " ", 1, -1, 1)
Do While Temp_STR <> Temp_STR_2
    Temp_STR_2 = Replace(Temp_STR, "  ", " ", 1, -1, 1)
    Temp_STR = Replace(Temp_STR, "  ", " ", 1, -1, 1)
Loop

Filter_String = "((Country_Name is Not NULL "
If Len(Temp_STR) > 0 Then
    Search_Words = Split(Temp_STR, " ")
    
    If UCase(Search_Words(0)) = "NOT" Then
        If UBound(Search_Words) > 0 Then
            Filter_String = Filter_String & ") AND (( Country_Name NOT LIKE '" & Like_Type & Search_Words(1) & Like_Type & "'"
            z = 2
        End If
    Else
        Filter_String = Filter_String & ") AND (( Country_Name LIKE '" & Like_Type & Search_Words(0) & Like_Type & "'"
            z = 1
    End If
    
    For i = LBound(Search_Words) + z To UBound(Search_Words)
        If Search_Words(i) <> "" And UCase(Search_Words(i)) <> "OR" And UCase(Search_Words(i)) <> "AND" And UCase(Search_Words(i)) <> "NOT" Then
            If UCase(Search_Words(i - 1)) = "OR" Then
                Filter_String = Filter_String & ") OR (Country_Name LIKE '" & Like_Type & Search_Words(i) & Like_Type & "'"
            ElseIf UCase(Search_Words(i - 1)) = "NOT" Then
                    If i > LBound(Search_Words) + 1 Then
                        If UCase(Search_Words(i - 2)) = "AND" Then
                            Filter_String = Filter_String & ") AND (Country_Name "
                            Filter_String = Filter_String & " NOT LIKE '" & Like_Type & Search_Words(i) & Like_Type & "'"
                        End If
                    End If
            Else
                Filter_String = Filter_String & ") AND (Country_Name LIKE '" & Like_Type & Search_Words(i) & Like_Type & "'"
            End If
        End If
    Next
End If
...
Build_Boolean_Criteria = Filter_String
End Function
...
Private Sub B_Search_Click()
    ...
    RS.Open "Select * from t_Cur Where " & Build_Boolean_Criteria(Trim(T_Search)), CN, adOpenStatic, adLockReadOnly
...
On Error GoTo Err_1
    ...
    With RS
        ...
        Do While Not .EOF And Temp_Counter < 13
           ' DoEvents
            Set LI = LV_Results.ListItems.Add(, "K" & !Cur_ID, .AbsolutePosition)
